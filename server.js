...
let globalDesktopWs = null; // global desktop slot — no sessionId needed

wss.on("connection", (ws, req) => {
  const isDesktop = req.headers["x-client"]?.includes("KJCArenaDesktop");
  let sessionId = null;
  let isGlobalDesktop = false;
  ...
  ws.on("message", (data) => {
    try {
      ...
      // Global desktop registration (new desktop app)
      if (msg.type === "REGISTER_DESKTOP") {
        isGlobalDesktop = true;
        globalDesktopWs = ws;
        ws.send(JSON.stringify({ type: "REGISTERED_DESKTOP", version: "1.0" }));
        console.log("Global desktop registered");
        // Notify all browser sessions
      ...
      if (msg.type === "REGISTER") {
        ...
        if (isDesktop) {
          wsSessions.get(sessionId).desktop = ws;
        } else {
          wsSessions.get(sessionId).browser = ws;
          const hasDesktop = wsSessions.get(sessionId).desktop?.readyState === 1 || globalDesktopWs?.readyState === 1;
          if (hasDesktop) ws.send(JSON.stringify({ type: "DESKTOP_CONNECTED" }));
        }
      ...
      // Route messages
      if (isGlobalDesktop) {
        // Global desktop sending results — route to correct browser by sessionId in message
        const targetSid = msg.sessionId;
        if (targetSid && wsSessions.has(targetSid)) {
          const pair = wsSessions.get(targetSid);
      ...
      if (sessionId && wsSessions.has(sessionId)) {
        ...
        if (isDesktop) {
          ...
          // Browser -> route to global desktop or session desktop
          const target = pair.desktop?.readyState === 1 ? pair.desktop : globalDesktopWs;
          if (target?.readyState === 1) {
            const withSession = { ...msg, sessionId };
            target.send(JSON.stringify(withSession));
          }
        }
      }
  ...
  ws.on("close", () => {
    if (isGlobalDesktop) {
      globalDesktopWs = null;
      console.log("Global desktop disconnected");
      for (const [sid, pair] of wsSessions) {
...
app.get("/health",(req,res)=>res.json({status:"ok",sessions:wsSessions.size,desktopConnected:!!(globalDesktopWs?.readyState===1)}));
